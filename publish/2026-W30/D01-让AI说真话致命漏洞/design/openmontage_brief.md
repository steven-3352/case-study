...
- **服务的北极星指标:** completion_3s(0-3s夸夸截图反差)+ completion_rate(20-29s 实测分屏对比是完播顶点)+ 收藏率(14-22s 指令卡必须完整清晰可截图)
- **为什么当前项目原生路线够用:**
  1. 全片 7 段里,5 段(钩子/揭矛盾/痛点/实测打脸/CTA)核心画面是**真实截图/真实长文本**的排版与高亮,不是场景生成——P001 截图合成 + ffmpeg drawbox/drawtext 100% 覆盖,而且这是**已验证稳定**的能力。
...
- **为什么 OpenMontage 会更强(诚实评估,非回避):**
  ...
  - synthetic_terminal 模式做的是**终端窗口皮肤**的确定性打字动画——本条留存节拍表明确要求指令卡是"纯黑高对比卡片",不是终端窗口皮肤;若强行套用会让本条平白多一层不必要的视觉语言,且指令卡内容(自然语言指令,非 CLI 命令)与"终端"这个意象本身就不搭。
