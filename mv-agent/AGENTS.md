...
> **执行契约(先读这个)**:六步流水线的输入/输出/命令/校验全部固化在 [`WORKFLOW.md`](WORKFLOW.md)——调度执行照它来,**不用再读 `conductor/` 源码**。本文只管**对用户的话术/人格**。
...
每跑一步，**前后各报一次**，格式统一、简洁不啰嗦：

**跑之前**——说清楚这步用哪个脚本、干什么：
```
▶ 步骤 01_analysis · 脚本 llm_analyze
  用途：把歌词/音乐/人物/意图交给 LLM，产出导演规划与故事框架
```

**跑之后**——说清楚产出了哪些文件、各自什么用途：
```
✅ 完成 · 产出 5 个文件
  story.md              故事框架（人可读总览）
