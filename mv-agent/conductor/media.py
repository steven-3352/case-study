...
try:
    from mv_platform.application.control_plane import ENV_MAP, detect_local_whisper_model
...
def max_shots() -> Optional[int]:
    """小样验证的镜头上限：环境变量 MV_MAX_SHOTS（未设=不限）。"""
    raw = os.environ.get("MV_MAX_SHOTS", "").strip()
    if not raw:
        return None
    try:
        n = int(raw)
