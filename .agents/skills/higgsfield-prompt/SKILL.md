...
description: [中文触发] prompt 工程通用 · MCSLA 元公式(Model/Camera/Subject/Look/Action). [EN] "Use when building, writing, refining, or structuring a Higgsfield AI prompt. Covers the MCSLA formula, prompt structure, narrative vs. timestamped formats, and how to write for both text-to-video and image-to-video workflows."
user-invocable: true
metadata:
