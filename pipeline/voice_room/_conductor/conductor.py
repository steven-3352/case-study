...
class Conductor:
    ...
    def init_project(self, tier: str = "standard") -> None:
        ...
        for spec in STEPS:
            ...
            for name in spec.prompts:
                ...
                if not pf.exists():
                    pf.write_text(f"# {name}\n(提示词占位 · 用户/agent 在此写中文)\n",
                                  encoding="utf-8")
        self.state.init(self.root.name, STEP_ORDER, tier)

    # ---- 完成判定:产物齐即算可完成(stub 阶段无 gate) ----
    ...
    def run_step(self, spec: StepSpec) -> dict:
        ...
        prompt_file = spec.prompts[0] if spec.prompts else None
        res = spec.tool(
            inputs=[sub["input"]],
            out_dir=sub["step"],
            params={"outputs": spec.outputs},
            prompt_file=prompt_file,
        )
        layout.append_log(self.root, spec.step_id,
    ...
    def reject(self, sid: str, feedback: str = "", units: list[str] | None = None) -> dict:
        ...
        (sub["meta"] / "feedback.md").write_text(feedback or "(无具体意见)", encoding="utf-8")
