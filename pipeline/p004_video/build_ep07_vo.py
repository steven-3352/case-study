#!/usr/bin/env python3
"""EP07 连载收官 · 真声主轨拼接(复刻 EP01)。

对 8 段真人录音去首尾静音 + 统一响度,按拍插入自然气口,拼成 master VO,
并输出每拍精确起止时间(供分镜 storyboard 各场景对齐)。

用法: ./.venv/bin/python pipeline/p004_video/build_ep07_vo.py
"""
from __future__ import annotations
import json, pathlib, subprocess, sys

ROOT = pathlib.Path(__file__).resolve().parents[2]
SRC = ROOT / "publish/2026-W29/连载-把AI调教成我的助理/脚本/EP07"
OUT = pathlib.Path(__file__).resolve().parent / "out" / "ep07full"
OUT.mkdir(parents=True, exist_ok=True)
SR = 48000

# (源文件, beat_id, 拍后气口秒)
BEATS = [
    ("前3秒.m4a",  "hook", 0.55),  # 开场给货钩子
    ("第一段.m4a", "b1",   0.55),  # 拍1 回望弧线(温、走过来的踏实)
    ("第二段.m4a", "b2",   0.45),  # 拍2 复刻步①②
    ("第三段.m4a", "b3",   0.55),  # 拍3 复刻步③④⑤
    ("第四段.m4a", "b4",   0.65),  # 拍4 效果(克制)→ 转局限
    ("第五段.m4a", "b5",   0.70),  # 拍5 诚实局限四条
    ("第六段.m4a", "b6",   0.95),  # 拍6 自警金句(慢、念完留白)★顶点
    ("第七段.m4a", "b7",   0.60),  # 拍7 收官 CTA(平视邀请、尾音不上扬)
]
...
def main():
    ...
    for fn, bid, _gap in BEATS:
        src = SRC / fn
        if not src.exists(): sys.exit(f"缺录音 {src}")
