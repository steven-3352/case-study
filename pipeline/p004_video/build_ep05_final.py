#!/usr/bin/env python3
"""EP05 全长版 · 最终合成(复刻 EP01/EP04)。

帧序列(6场景)→ 静音串接 → overlay 字幕 PNG 序列 → 混 VO(真声)+ SFX 床 → 成片。
用法: ./.venv/bin/python pipeline/p004_video/build_ep05_final.py
"""
...
OUT = ROOT/"out"/"ep05full"
SCENES = OUT/"scenes"; SCENES.mkdir(parents=True, exist_ok=True)
FPS, W, H = 30, 1080, 1920
# 带 ep05_ 前缀,避免与 EP01-04 旧帧共用 out/frames/<id> 目录串帧
SCENE_IDS = ["ep05_s0_hook","ep05_s1_seed","ep05_s2_locks12","ep05_s3_locks34",
             "ep05_s4_honest","ep05_s5_land"]
VO = OUT/"vo_master.wav"
SFX = OUT/"sfx_bed_full.wav"
SUBS = FRAMES/"ep05f_subs"
...
# 也出一版无 sfx(纯真声+字幕)备份
final_vo = OUT/"EP05_全长版_真声纯VO_抖音.mp4"
