#!/usr/bin/env python3
"""EP02 全长版 · 最终合成(复刻 EP01)。

帧序列(8场景)→ 静音串接 → overlay 字幕 PNG 序列 → 混 VO(真声)+ SFX 床 → 成片。
用法: ./.venv/bin/python pipeline/p004_video/build_ep02_final.py
"""
...
OUT = ROOT/"out"/"ep02full"
SCENES = OUT/"scenes"; SCENES.mkdir(parents=True, exist_ok=True)
FPS, W, H = 30, 1080, 1920
# 带 ep02_ 前缀,避免与并行 EP03 任务共用 out/frames/<id> 目录串帧
SCENE_IDS = ["ep02_s0_hook","ep02_s1_endure","ep02_s2_ask","ep02_s3_fakefilm",
             "ep02_s4_unaware","ep02_s5_read","ep02_s6_land","ep02_s7_recap"]
VO = OUT/"vo_master.wav"
SFX = OUT/"sfx_bed_full.wav"
SUBS = FRAMES/"ep02f_subs"
...
# 也出一版无 sfx(纯真声+字幕)备份
final_vo = OUT/"EP02_全长版_真声纯VO_抖音.mp4"
