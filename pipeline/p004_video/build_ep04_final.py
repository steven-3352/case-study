#!/usr/bin/env python3
"""EP04 全长版 · 最终合成(复刻 EP01/EP02)。

帧序列(8场景)→ 静音串接 → overlay 字幕 PNG 序列 → 混 VO(真声)+ SFX 床 → 成片。
用法: ./.venv/bin/python pipeline/p004_video/build_ep04_final.py
"""
...
OUT = ROOT/"out"/"ep04full"
SCENES = OUT/"scenes"; SCENES.mkdir(parents=True, exist_ok=True)
FPS, W, H = 30, 1080, 1920
# 带 ep04_ 前缀,避免与 EP01-03 旧帧共用 out/frames/<id> 目录串帧
SCENE_IDS = ["ep04_s0_hook","ep04_s1_scatter","ep04_s2_rule3","ep04_s3_heads",
             "ep04_s4_auto","ep04_s5_graph","ep04_s6_parallel","ep04_s7_land"]
VO = OUT/"vo_master.wav"
SFX = OUT/"sfx_bed_full.wav"
SUBS = FRAMES/"ep04f_subs"
...
# 也出一版无 sfx(纯真声+字幕)备份
final_vo = OUT/"EP04_全长版_真声纯VO_抖音.mp4"
