#!/usr/bin/env python3
"""EP06 全长版 · 最终合成(复刻 EP01/EP05)。

帧序列(9场景)→ 静音串接 → overlay 字幕 PNG 序列 → 混 VO(真声)+ SFX 床 → 成片。
用法: ./.venv/bin/python pipeline/p004_video/build_ep06_final.py
"""
...
OUT = ROOT/"out"/"ep06full"
SCENES = OUT/"scenes"; SCENES.mkdir(parents=True, exist_ok=True)
FPS, W, H = 30, 1080, 1920
# 带 ep06_ 前缀,避免与其它集旧帧共用 out/frames/<id> 目录串帧
SCENE_IDS = ["ep06_s0_hook","ep06_s1_stance","ep06_s2_crash1","ep06_s3_fix1",
             "ep06_s4_crash2","ep06_s5_fix2","ep06_s6_crash3","ep06_s7_boundary",
             "ep06_s8_land"]
VO = OUT/"vo_master.wav"
SFX = OUT/"sfx_bed_full.wav"
SUBS = FRAMES/"ep06f_subs"
...
# 也出一版无 sfx(纯真声+字幕)备份
final_vo = OUT/"EP06_全长版_真声纯VO_抖音.mp4"
