#!/usr/bin/env python3
"""EP07 收官全长版 · 最终合成(复刻 EP01/EP05)。

帧序列(8场景)→ 静音串接 → overlay 字幕 PNG 序列 → 混 VO(真声)+ SFX 床 → 成片。
用法: ./.venv/bin/python pipeline/p004_video/build_ep07_final.py
"""
...
OUT = ROOT/"out"/"ep07full"
SCENES = OUT/"scenes"; SCENES.mkdir(parents=True, exist_ok=True)
FPS, W, H = 30, 1080, 1920
# 带 ep07_ 前缀,避免与 EP01-06 旧帧共用 out/frames/<id> 目录串帧
SCENE_IDS = ["ep07_s0_hook","ep07_s1_recap","ep07_s2_steps12","ep07_s3_steps345",
             "ep07_s4_effect","ep07_s5_limits","ep07_s6_creed","ep07_s7_cta"]
VO = OUT/"vo_master.wav"
SFX = OUT/"sfx_bed_full.wav"
SUBS = FRAMES/"ep07f_subs"
...
# 也出一版无 sfx(纯真声+字幕)备份
final_vo = OUT/"EP07_全长版_真声纯VO_抖音.mp4"
