...
DEFAULT_VO = AUDIO / "vo_v0.mp3"
DEFAULT_SUB = ROOT / "sub_v0.srt"
...
def load_storyboard(path: pathlib.Path) -> tuple[dict, list[SceneSpec]]:
    with path.open(encoding="utf-8") as f:
        sb = yaml.safe_load(f)
    scenes = [SceneSpec(id=s["id"], duration=float(s["duration"])) for s in sb.get("scenes", [])]
...
def compose_final(
    ...
    若 VO 比视频长,自动 freeze 最后一帧补足。
    """
    video_dur = dur(video)
    vo_dur = dur(vo)
    final_dur = max(video_dur, vo_dur)
    ...
    # 视频链 · 若需补尾,先 tpad freeze 最后一帧
    if extra > 0.05:
        v_pad = f"[0:v]tpad=stop_mode=clone:stop_duration={extra:.2f}[v0p];"
        v_main = "[v0p]"
    else:
        v_pad = ""
        v_main = "[0:v]"
    ...
    if sub_idx >= 0:
        v_chain = f"{v_pad}{v_main}[{sub_idx}:v]overlay=0:0:format=auto:shortest=0[v]"
    else:
        v_chain = f"{v_pad}{v_main}copy[v]" if extra > 0.05 else "[0:v]copy[v]"

    # 音频链 · 用 final_dur 让 BGM fade out 落在 VO 结束前
    if bgm_idx >= 0:
        fade_out_start = max(0.0, final_dur - 2.0)
        a_chain = (
            ...
            f"[vo][bgm]amix=inputs=2:duration=longest:dropout_transition=0:normalize=0[a]"
        )
    else:
...
def main() -> None:
    ...
    scene_mp4s = [scene_to_mp4(s, fps, width, height) for s in scenes]
