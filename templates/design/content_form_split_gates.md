...
| 推荐方案 | 选中方案 + 不选其他方案的原因 |
| **时刻类型**（注意力层 · 硬门） | 本镜属 停划/看懂/交接/收藏钩/评论钩 哪一类（全片 ≥2 类） |
| **单焦点**（注意力层 · 硬门） | 本镜唯一视线落点；其余压灰/虚化/不动退背景层（≥3 镜声明） |
| **加料不加赢家**（注意力层 · 硬门） | 流行元素/特效做成容器或背景层，非第二赢家 + 理由 |

> **注意力引导系统**（form_strategy 下沉层 · 2026-06-26 直接硬门）：管「表达有没有制造单一视线路径」，补 form_strategy「选哪种表达」之下一级。两根轴（细节密度 × 焦点数）→ 目标格「单焦点 + 高细节」；炫技毙否测试 = 打散注意力即毙。全规与手法库见 `templates/design/attention_guidance_system.md`，`gate_check.check_form_strategy()` fail-closed。

**门禁：** 视频/强互动图文缺 `form_strategy`，或只写“用 P004 / 用 Three / 用 B-roll”而没有数据杠杆，形式门默认 blocked。

### 13.2 视觉语言策展师职责

在 `form_strategy.md` 后、`storyboard.yaml` 前，视觉语言策展师必须输出 `design/design_language.md`，把 DESIGN.md / 成熟网站视觉系统转成**本条可执行约束**：

| 字段 | 必填内容 |
|------|----------|
| 参考来源 | 使用哪个 DESIGN.md / 网站视觉系统，为什么适合本条目标 |
| 色板 | 至少 4 个 hex 色值，声明 canvas/surface/ink/muted/accent 等角色 |
| 字体层级 | display/headline/body/caption/data 的字号、字重、行高或替代规则 |
| 组件规则 | 封面标题、信息卡、表格/字段表、对比块、CTA、标签如何画 |
| 逐镜/逐页应用 | 每个关键镜头/页面使用哪些 token/组件，焦点路径是什么 |
| Don't | 禁用项：照抄品牌、accent 泛滥、随机渐变、无意义装饰、文字拥挤等 |
| 像素验收 | 下游 HTML/PNG/视频如何证明视觉语言被兑现 |

**门禁：** 缺 `design/design_language.md`，或只写“高级感 / 产品感 / 干净一点”而无 token 与逐镜应用 → 禁止 storyboard 定稿 / render。

### 13.3 Web 3D / 高级动效使用规则
...
### 13.5 OpenMontage 外部制作路由（可选插件 · 默认关闭）

> **定位：** OpenMontage 是高阶视频制作插件，不是内容生产大脑。当前项目仍负责选题、洞察、脚本、留存、形式策略、视觉语言和最终外发判断。

当 `form_strategy.md` 与 `design_language.md` 已完成，且原生 P004/P007 路线不足以兑现画面任务时，可由 **OpenMontage 制作导演**填写：
...
| 场景 | 合法理由 |
|------|----------|
| 真实素材蒙太奇 / 纪录片感 | 能提升证据感、语境进入速度或中段视频感 |
| 参考视频节奏拆解 | 能服务 0–3s 停划或完播节拍，而非照搬 |
| Remotion / HyperFrames / FFmpeg 复杂合成 | 原生 HTML/GSAP 难以稳定兑现 |
| 屏幕录制 + B-roll + 字幕混剪 | 比单一看板更快让用户看懂 |
| 产品广告 / 电影级预告试点 | 有明确数据假设和预算上限 |

#### 禁止启用

- 只是想“更酷 / 更电影感 / 更高级”。
- 脚本、价值锚、事实边界还未通过内容门。
- 试图让 OpenMontage 重写选题、卖点或 CTA。
- 小红书图文轮播、简单大字卡片、清单页等原生路线已足够的内容。
- 没有预算、素材来源、授权记录或回流验收标准。
...
publish/{week}/Dxx-*/xhs/
```

只有 `design/openmontage_review.md` 通过后，编导才可决定是否复制到平台目录。

#### 回流验收

OpenMontage 成片必须同时通过：

| 验收 | 标准 |
|------|------|
| 内容一致性 | 未改变 chosen script、价值锚、CTA、事实边界 |
| 画面承诺 | brief 中 0–3s / 中段 / 结尾承诺在 mp4 中可见 |
| 视觉语言 | `design_language.md` 的 token、组件、焦点路径被兑现 |
| 技术 QA | 无黑屏、静音异常、字幕错位、音频削波、素材丢失 |
| 授权记录 | `asset_log.md` 记录来源、授权、AI provider / prompt / 成本 |
| 路线对比 | 明显优于当前 P004/P007 原生路线，否则回退 |

**铁律：** OpenMontage 版本若没有明显优于原生路线，只能归档为试验，不得因为“已经制作完成”而外发。

## 14. 双平台分轨（W26 数据 · 2026-06-25）

> **W26 实测：** 抖音 121 播 / 小红书 539 曝 46 观 0 藏 · `video_reuse` 全弱。自 W27 起执行。

| # | 铁律 | 执行 |
|---|------|------|
| 1 | **禁止 mp4 直复用** | `meta.yaml` / `verdict.yaml` 不得 `delivery: video_reuse` |
| 2 | **分平台立项** | 同痛点可成对，但须独立脚本 + `douyin/publish.md` / `xhs/publish.md` |
| 3 | **平台默认形态** | 抖音：38–45s 叙事视频；小红书：轮播/清单/字段表 |
| 4 | **日更分轨** | 每平台每天 1 条 · 独立 publish.md · xhs 默认 12:30 / dy 19:30 |
| 5 | **叙事优先于换皮** | D02 相对最佳来自单场景+原话+CTA，非 F3 UI 换皮 |

详规：`docs/SYSTEM.md` §3.1e · `publish/2026-W27/PLAN.md` · `ops/rules.yaml` R08–R12
