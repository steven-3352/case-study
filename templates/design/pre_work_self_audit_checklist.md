...
- **⭐【铁律 8】门禁是地板不是目标 —— 抬高 3 档再验收。** 我此刻感到"这个能过门禁"，那个"能过"的感觉本身就是"标准定低了"的信号。强制自问：**能不能把验收目标往上提 3 个档次？** 若能，就把抬高后的标准当成真正的验收目标去建/去评，而不是停在刚过线。"3 档"是刻意的实质性大跳（不是 +1 敷衍），逼自己从"最低可接受"切到"什么才算明显出色"。凡放行理由是"它过了 gate"→ 停，问"过的是目标还是代理指标"。**本项目所有制作视频的 workflow 每一道验收都必须过这一条。**（来源 memory `feedback_gate-floor-not-target` · `feedback_build-to-reference-not-floor`；语音厅 MV 事故：只建到"190px 位移底线"就停，过了所有自建检查却单调如 PPT）
...
**核心原则：** 不要等用户发现——被发现之前自己先挑出来。**门禁只回答"够不够格"，不回答"好不好"；过线只是起点不是终点。**
