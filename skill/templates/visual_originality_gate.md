...
> 工种：**形式策略官 + 视觉语言策展师 + 动画导演**（`skill/roles/registry.yaml`）
> 位置：`design/visual_originality_gate.md`
> 时机：`form_strategy` 之后、`storyboard` 定稿之前。
> 核心原则：**脚本结构可以复用，表现形式不能模板化。模板只能约束判断流程，不能决定画面长什么样。**
> 对应质量门：**`QG-VISUAL-ORIGINALITY`**（本文件即其判定载体：6 必答 + 5 must-have + 6 fail 条件）· 关联 **`QG-FORM-TEMPLATE-REPEAT`**（同模板 ≤1 镜）· **`QG-FORM-DISTINCT`**（48s+ ≥6 种模板）。基准是地板，按 `QG-RAISE-3` 提升 3 档目标验收。

## 0. 入口必读（开工前打勾）

> 流程与门位见 `skill/docs/PROCESS.md`；不过清单不得开工

- [ ] **流程 refs**：`skill/docs/PROCESS.md`（形式→分镜门位顺序）
- [ ] **质量 refs**：`skill/quality/quality_registry.md`（`QG-VISUAL-ORIGINALITY` · `QG-FORM-TEMPLATE-REPEAT` · `QG-FORM-DISTINCT` · `QG-RAISE-3`）
- [ ] **template refs**：本节点已完成 `form_competition.md` + form_strategy + `design_language.md`（原创差异必须能对应到具体 token）
- [ ] **历史成品参考**：**最近约 5 条**的 storyboard 首屏/中段/CTA 实读（不看是抄错的起点）+ 项目形式失败记录
...
## 3. 模板/组件复用声明（`QG-FORM-TEMPLATE-REPEAT`：同模板 ≤1 镜）

只要 storyboard 出现复用模板（`template: xxx`），该镜头必须同时包含：
...
未通过时，不允许 storyboard 定稿，不允许 TTS、文生图、render。
