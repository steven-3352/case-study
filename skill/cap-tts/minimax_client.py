"""MiniMax TTS 客户端 · speech-2.8-turbo + t2a_async_v2(能力封装版).

凭证:MINIMAX_API_KEY、MINIMAX_BASE_URL(三方中转)、MINIMAX_GROUP_ID(可选)。
封装自 pipeline/tts/minimax_client.py,仅把 `pipeline.env_loader` 改为本地 local_env。
"""
...
from local_env import api_base
...
def download_file(file_id: int, cfg: dict, *, key: str, group_id: str | None) -> bytes:
    """取异步任务音频。云雾中转返回 tar 包需解 mp3;官方裸二进制也兼容。"""
    host = _minimax_host(cfg)
