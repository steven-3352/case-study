...
## W26D08 · douyin rebuild v2 · 2026-06-25

- **纸面分：** 初版不计；按用户反馈，重做版明显优于旧版
- **诚实效果分：** pass（形式丰富度和选题贴合度显著提升）
- **有效做法：**
  - 首镜用真实店主/店铺素材 + 大字 Hook，先建立电商老板语境
  - 痛点镜不用解释卡，改成私域消息瀑布，让“客户混在一起”可视化
  - 方案镜把 Agent 拆成分层、提醒、预警三个职责，而不是泛讲 AI
  - 证据镜做“今日待跟进”运营看板，让用户看到人数、下一步、人工优先
  - 中后段加入风险雷达，打断连续卡片疲劳
  - 末尾用四选项 CTA，让评论动作具体化
- **校验方法：**
  - `ffprobe` 验证 1080×1920、30fps、45s、有音频
  - `ffmpeg select` 抽 6 张关键帧：首镜、消息、分工、看板、风险、CTA
  - 人眼检查旧字幕残留、黑屏、素材路径、文字溢出、旧模板感
  - 首次发现旧字幕和首镜背景路径问题后，必须重新合成，不用解释带过
- **防旧模板干扰：**
  - 不从旧 video/card 模板改文案，而从“这一镜要让观众做什么”开始
  - storyboard 引用真实落地的 `assets/broll/...` 和新建 `d08_*.html`
  - 把旧通用渲染路线降级为草稿/参考，不作为外发路线
  - 文档承诺不算，最终关键帧看见才算
- **进化：**
  - `docs/SYSTEM.md` §3.1d
  - `templates/design/content_form_split_gates.md` §12
