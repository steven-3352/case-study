...
> **北极星优先：** 工具选型服从 `docs/SYSTEM.md` §1.0 · §4.2 — 先定这一镜的停划/看懂/互动，再选实现；仓库里有什么不是理由。
>
> Phase 0–1 能手工就不写代码；Phase 2+ 再脚本化。不追求完美架构。
...
> 完整流程：`docs/SYSTEM.md` §4.2。本节为技术 tie-breaker。

| 需求 | 首选 | 原因（对北极星） |
|------|------|------------------|
| 0–3s 停划 + 钉子场景 | 真实 B-roll / 实拍 | 信任感、拇指停住 |
| 证明系统真的在跑 | P001 截图/录屏 · `produce.py` | 看懂快、证据强（Q9） |
| 大字节拍、对比、网格、漫画分镜 | HTML + GSAP（P004–P007） | 字可读、与 VO 时间轴对齐 |
| 真 3D 空间、相机轨道、大量粒子 | Three.js | 仅当 2D/CSS 明显假且停划需要 |
| 小红书收藏型轮播 | P007 漫画 / P002 报纸 | 看本条收藏动机，非习惯 |
| 带货质感 | 真人脸/手/产品 | Q8 |

**GSAP vs Three.js（都能做动效时）：**

- 默认 **GSAP** — 现有 `capture_frames.py` 时间轴、DOM 字幕叠层、中文排版
- **Three.js** 仅当：画面本质是 3D 空间问题 + forecast 预估停划/看懂会优于 2D + 不会因新管线出赶工烂版
- 若真实素材能传达同样信息 → **两者都不选**，用 P001/B-roll
