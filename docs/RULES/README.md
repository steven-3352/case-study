...
1. 用户明确选择的入口工作流约束优先级最高。
2. 项目根通用约束仅在选定工作流未定义时补充适用。
3. 未选中的入口工作流规则不得加载、继承、引用或执行。
4. `mv-agent`、`ad-agent`、`ad2-agent` 三套约束彼此隔离；同名能力也必须按当前入口实现，不得跨工作流套用门禁、默认值、提示词或降级路线。
5. 用户未明确入口时必须先让用户从三个入口中选择，禁止代选。
