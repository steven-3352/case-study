...
**约定(owner 2026-08-09 拍板):所有新项目物料目录位于 `/home/ubuntu/mvstudio-workspace/` 下；新项目的"用户项目目录"= 用户原始物料文件所在的子目录。**
...
5. **固定物料父目录:**新用户物料统一放在 `/home/ubuntu/mvstudio-workspace/<物料目录>/`,不得为单片另找父目录。
6. **固定公网映射:**Nginx 的 `/seedance-assets/` 固定 alias 到 `/home/ubuntu/mvstudio-workspace/`;公网基址为 `http://192-144-235-54.sslip.io/seedance-assets/`。公开 URL 直接拼接物料根的相对路径,不得为单片重新探测或修改 Nginx。
